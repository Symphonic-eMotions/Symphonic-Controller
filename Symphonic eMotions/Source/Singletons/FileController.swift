...
class FileController: ObservableObject {
    ...
    func addDirectoryURLsToController() -> [URL] {
        loadedURL = getContentsOfDirectory()
    ...
    func fileNameOrCustomName(url: URL, fileName: String) -> String {
    ...
    func setNameCustomName(url: URL) -> String {
    ...
    func fileContents(url: URL) -> InstrumentsSet? {
    ...
    func urlToPlayListFileName(url: URL) -> String {
    ...
    func nameFromUrl(url: URL) -> String {
        let nameParts = urlToNameParts(url: url)
        if nameParts.count > 1 {
    ...
    func deleteFile(url: URL) -> [URL] {
        ...
        return getContentsOfDirectory()
    }
}

extension URL {
    var lastTwoPathComponents: String {
        let parentDirectoryName = deletingLastPathComponent().lastPathComponent
