...
extension Conductor {
    func midiNoteNumberFromFileName(_ fileName: String, separators: [Character] = ["_", "-", "/"]) -> Int? {
...
//        print("Note: \(note) - Octave: \(octave)")
    ...
    func lengthInBeatsFromFileName(fileName: String, separators: [Character] = ["_", "-", "/"]) -> Double? {
        let components = fileName.split { separators.contains($0) }
