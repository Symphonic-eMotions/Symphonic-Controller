...
final class Conductor: HasAudioEngine {
    ...
    var trackMixers: [String: Mixer] = [:]
    var trackRecorders: [String: NodeRecorder] = [:]

    // Amplitude controller with interpolation per track
    var amplitudeControllers: [String: AmplitudeController] = [:]
...
//            effectAmpEnvs?[setEffect.id] = envelope
    ...
    func envelopeSamplerPlay(effectType: EffectType, identifier: Int) -> UUID? {
        guard let uuid = uuid(for: effectType, identifier: identifier),
              let sampler = effectSamplers?[uuid]
        else {
            return nil
        }
...
//                trackAmpEnvelopes[trackId]!.scheduleMIDIEvent(event: envOn)
    ...
    func copyMidiSingleTrack(trackId: String, nextVariation: Int, loopLength: [Double]) {
    ...
    func playNoteNumberSingleTrack(
        trackId: String,
        soundSource: InstrumentsSet.Track.InstrumentType,
        noteNumber: Int,
        noteOn: Bool
...
//            trackAmpEnvelopes[trackId]!.scheduleMIDIEvent(event: trackOn)
    ...
    func playNoteNumbersIntroduction(
        trackId: String,
        soundSource _: InstrumentsSet.Track.InstrumentType,
        noteNumbers: [Int],
        noteOn: Bool
    ) {
        if !noteOn {
...
//            trackAmpEnvelopes[trackId]!.scheduleMIDIEvent(event: trackOn)
            ...
            if let noteNumber = noteNumbers.randomElement() {
                ...
                print("EEROR PLAYING. last was \(String(describing: lastNoteNumber))")
            }
    ...
    func stopAllNoteNumbers(trackId: String) {
        for noteNumber in 0 ... 127 {
            let noteOff = MIDIEvent(noteOn: MIDINoteNumber(noteNumber), velocity: MIDIVelocity(0), channel: 1)
...
//            }
    ...
    func setTempo(tempoChange: Double) -> Double {
        currentTempo = currentTempo + tempoChange
...
//    }
    ...
    private func levelMidiClipVariation(in level: Int, on track: TrackSettings) {
    ...
    private func levelNoteNumberVariation(in level: Int, on track: TrackSettings) {
        if track.levels.contains(level) {
    ...
    func calculateMIDIstartTime(
        for currentTimeScore: Int,
        in loopLengths: [Double]
    ) -> Double {
        var currentMIDIstartTime = 0.0
    ...
    func copyMIDIfromMemory(trackId: String, midiStartTime: Double, loopLength: Double) {
    ...
    func stopNotesTrackId(for trackId: String) {
    ...
    func pauzeEngineAndStopTracks(
        ...
        for value in setSettings.tracks.values {
            stopTrack(value)

            for noteNumber in 0 ... 127 {
                stopSamplerNote(value, noteNumber)
            }
        }
