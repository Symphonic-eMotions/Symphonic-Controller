...
struct AreaValues {
    ...
    func withNewRawValue(_ value: Int, maxValue: Int, feedback: Float) -> AreaValues {
        ...
        let feedbackValue = min(newScaledValue + (Double(average) * Double(feedback)), 1.0)
...
extension AreaValues {
    init(value: Int, maxValue: Int) {
        ...
        previousScaledValue = initialScaledValue
        recentValues = [Double(value)]
        rawDifference = value
...
extension DoubleValues {
    init(value: Int, maxValue: Int) {
        scaledValue = Double(value) / Double(maxValue)
        recentValues = [Double(value)]
        rawDifference = value
        self.maxValue = maxValue
    }
